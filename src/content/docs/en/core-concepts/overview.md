...
- Edge proxy layer: the API gateway reverse-proxies requests through Cloudflare's global network. Client requests first arrive at Cloudflare edge nodes, pass compliance and rate-limit checks, then fetch raw data from the main site.
...
- Console responsibility: Console is only the authentication center for managing API tokens and viewing usage quotas. It does not maintain or modify data.

## Documentation structure

| Section | Purpose |
| :--- | :--- |
| Start pages | Explain authentication, rate limiting, and unified error handling. |
