...
你不再需要从脆弱的爬虫、不一致的数据格式和重复的底层接入工作开始。公开数据同步、统一字段约定、API token 认证、频次限制和高可用交付由平台负责；开发者可以把精力放在创意、体验、研究、可视化和真正有趣的功能上。
...
- 频次限制：认证失败流量使用基于 IP 的 `anonymous` 配额进行防滥用保护；认证成功流量按 API token 的有效配额限制。
- 默认配额：`anonymous` 为 30 requests / 60s，`default` 为 60 requests / 60s，`plus` 为 300 requests / 60s。用户级配额覆盖优先于 tier 默认配置。
- 重试行为：`Retry-After` 只会在 `429` 时返回；`X-RateLimit-Reset` 是 Unix seconds reset timestamp。
- 数据协议：除非另有说明，公开数据采用 CC BY 4.0；发布集成时应提供署名。

## 文档结构

- 开始：首页、快速开始、认证、频次限制、错误处理。
- 核心概念：概览、数据模型与层级。
- 通用规范：基础字段、响应结构。
...
当前文档已经定义平台形态、稳定模型词汇、通用响应结构、认证模型、频次限制和社区使用规则。下一步最值得补充的是：
